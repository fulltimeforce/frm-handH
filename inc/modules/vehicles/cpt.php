...
function cpt_vehicles_init() {
    ...
    $args = array(
        ...
        'show_in_rest'       => true,
        // esto hace que aparezcan los metaboxes de las taxonomías en el editor
        'taxonomies'         => array('vehicle_category', 'vehicle_brand'),
    );

    register_post_type('vehicles', $args);
}
add_action('init', 'cpt_vehicles_init', 0); // prioridad baja: primero el CPT
...
add_action('init', 'vehicles_register_tax_cat', 1); // después del CPT

// ===== Vehicle Brands (non-hierarchical, tags-like) =====
...
add_action('init', 'vehicles_register_tax_brand', 1); // después del CPT